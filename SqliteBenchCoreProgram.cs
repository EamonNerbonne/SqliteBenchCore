...
PerfTest("System.Data.SQLite.SqliteConnection(Dapper)",
    n => testDapper(new SQLiteConnection("Data Source=:memory:"), n));
...
PerfTest("Sql Server LocalDb(Microsoft.Data.SqlClient)",
    n => testADO(new SqlConnection("Data Source=(LocalDb)\\MSSQLLocalDB"), n));
PerfTest("Sql Server LocalDb(Microsoft.Data.SqlClient, Dapper)",
    n => testDapper(new SqlConnection("Data Source=(LocalDb)\\MSSQLLocalDB"), n));
...
static void PerfTest(string name, Func<int, int> countToSum)
{
    ...
    if (sum1 != 30) throw new("Invalid engine result");

    var sum = BestTime(count + 1, countToSum, timingRepeats, out var manyQueriesUS);
    if (sum != 30 * (count + 1)) throw new("Invalid engine result");
    ...
    using (conn)
    {
        ...
        using (var cmd = conn.CreateCommand())
        {
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                while (reader.Read()) sum += reader.GetInt32(0) + reader.GetInt32(1) + reader.GetString(2).Length;
            }
        }
    }
...
static int testRAW(int count)
{
    ...
    using (db)
    {
        if (rc != raw.SQLITE_OK) throw new();

        var errStmt = raw.sqlite3_prepare_v2(db, query, out var stmt);
        using (stmt)
        {
            if (errStmt != raw.SQLITE_OK) throw new();
